...
//if ( ! defined( 'ABSPATH' ) ) exit; 
    ...
    $items = $wpdb->get_results(
            "SELECT  gfform_id FROM ".$sql_gflabel_table." WHERE gflabel_name= '$label_name'");
	$data['result'] = $items;
...
if($gf_label_name == ''){
	...
	$count = $wpdb->get_var("SELECT COUNT(*) FROM ".$sql_gflabel_table." WHERE gflabel_name = '$gf_label_name'");
	$count_all = $wpdb->get_var("SELECT COUNT(*) FROM ".$sql_gflabel_table." WHERE gfform_id = ''");
	
	$label_listing_page = get_site_url() . '/wp-admin/admin.php?page=flgf_gform_folders';
...
<!--<input type="hidden" name="gfolders_gf" value="<?php // echo wp_create_nonce($_GET['t']); ?>" />-->
	...
	<div class="input-group" id="new-folder-name">
		...
		<input type="text" name="gf_label_name" placeholder="Type Folder Name" class="form-control gf_label_input" required>
	</div>

<p class="submit"><input type="submit" value="<?php _e('Create Folder', 'gfolders'); ?>" class="button button-primary" id="submit" name="submit"></p>

</form>
</div>

        <h1 class="wp-heading-inline"><?php echo esc_html("Gravity Form Folders"); ?> <!--<a class="page-title-action" href="admin.php?page=gform_add_label">Add New</a>--></h1> 
